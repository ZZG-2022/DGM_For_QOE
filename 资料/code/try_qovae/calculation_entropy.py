...
a, b, c, d, e, f, FF1, FF2, FF3, FF4, FF5, FFn, HH, GG1, GG2, GG3, GG4, GG5 = map(sp.IndexedBase,
                                                                                  ['a', 'b', 'c', 'd', 'e', 'f', 'FF1',
                                                                                   'FF2', 'FF3', 'FF4', 'FF5', 'FFn',
                                                                                   'HH', 'GG1', 'GG2', 'GG3', 'GG4',
                                                                                   'GG5'])
l, l1, l2, l3, l4, l5, l6, l7, l8, x1, x2, x3, x4, x5, x6, coeff, powern = map(sp.Wild,
                                                                               ['l', 'l1', 'l2', 'l3', 'l4', 'l5', 'l6',
                                                                                'l7', 'l8', 'x1', 'x2', 'x3', 'x4',
                                                                                'x5', 'x6', 'coeff', 'powern'])
...
def PartialTraceOne(expr, n):  ##calculate the partial trace such as A|BCD
    ...
    for ii in range(len(TermsCoeff)):
        ...
        if HHList.indices[n - 1] == HHList.indices[n + 3]:
            ...
            del (ll[n - 1],
                 ll[n + 2])  ## because cannot del all at the same time, thus do it one by one, the index is not n+2
    ...
    for ii in range(len(TermsCoeff)):
        HHList = TermsCoeff[ii][0]
        if HHList.indices[n - 1] == HHList.indices[n + 3]:
            ll = [HHList.indices[0], HHList.indices[1], HHList.indices[2], HHList.indices[3], HHList.indices[4],
                  HHList.indices[5], HHList.indices[6], HHList.indices[7]]
            del (ll[n - 1],
                 ll[n + 2])  ## because cannot del all at the same time, thus do it one by one, the index is not n+2
...
if __name__ == '__main__':
    example = ["OAMHolo(XXX,f,-2)", "BS(XXX,a,d)", "Reflection(XXX,f)", "BS(XXX,b,c)", "DownConv(XXX,1,a,d)",
               "DownConv(XXX,1,b,e)", "BS(XXX,a,d)", "OAMHolo(XXX,f,1)"]
    calculate(example)


def calculate_entropy_1(experimental_sequence):
    calculate(experimental_sequence)
